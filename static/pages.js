...
const familySupporterTable=[...main.querySelectorAll('table')].find(table=>table.querySelector('form[action^="/contacts/"]'));
if(familySupporterTable){familySupporterTable.id='family-supporters-table';const heading=familySupporterTable.closest('section')?.querySelector('.supporter-summary-heading');if(heading){const search=document.createElement('input'),label=document.documentElement.lang==='yi'?'זוכן העלפער':document.documentElement.lang==='he'?'חיפוש תומכים':'Search supporters';search.type='search';search.placeholder=label;search.setAttribute('aria-label',label);search.autocomplete='off';search.dataset.tableSearch=familySupporterTable.id;heading.lastElementChild?.classList.add('family-supporter-count');const tools=document.createElement('div');tools.className='family-supporter-tools';heading.append(tools);tools.append(search);}}
document.querySelectorAll('[data-table-search]').forEach(search=>{const table=document.getElementById(search.dataset.tableSearch),body=table?.tBodies[0];if(!body)return;const rows=[...body.rows];search.addEventListener('input',()=>{const query=search.value.trim().toLocaleLowerCase();rows.forEach(row=>row.dataset.searchMatch=String(!query||row.textContent.toLocaleLowerCase().includes(query)));table.dispatchEvent(new Event('table-search'));});});
