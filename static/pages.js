...
const revealCommunicationTarget=target=>{if(!target)return;const tools=target.closest('details.communication-tools');if(tools)tools.open=true;requestAnimationFrame(()=>{target.scrollIntoView({behavior:'smooth',block:'start'});target.focus?.({preventScroll:true});});};
