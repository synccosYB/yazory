...
document.querySelectorAll('[data-select-filter]').forEach(search=>{const select=document.getElementById(search.dataset.selectFilter);if(!select)return;const filter=()=>{const query=search.value.trim().toLocaleLowerCase();for(const option of select.options){const wrongFamily=supporterFamily&&select===supporterParent&&option.dataset.familyId&&option.dataset.familyId!==supporterFamily.value;option.hidden=Boolean(wrongFamily||(query&&!option.textContent.toLocaleLowerCase().includes(query)));}if(select.selectedOptions[0]?.hidden)select.value='';};search.addEventListener('input',filter);supporterFamily?.addEventListener('change',()=>{search.value='';filter();});});
