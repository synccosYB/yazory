...
const candidates=[...main.children].filter(el=>el.matches('section.card')&&!el.hasAttribute('data-panel-exclude')&&el.querySelector('h2'));
...
if(familySupporterTable){
  familySupporterTable.addEventListener('click',event=>{
    const actions=event.target.closest('.supporter-contact-actions');
    if(!actions?.open)return;
    if(event.target.closest('[data-supporter-contact-close]') || event.target===actions){
      actions.open=false;
      actions.querySelector(':scope > summary')?.focus();
    }
  });
  document.addEventListener('keydown',event=>{
    if(event.key!=='Escape')return;
    const actions=familySupporterTable.querySelector('.supporter-contact-actions[open]');
    if(actions){actions.open=false;actions.querySelector(':scope > summary')?.focus();event.preventDefault();}
  });
}
...
main.querySelectorAll('section.card').forEach(section=>{let rows=[...section.children].filter(el=>el.matches('[data-page-item],.activity-row'));if(rows.length<2)return;let page=0;const focusId=new URLSearchParams(location.search).get('staff_id');let focusRow=focusId?rows.findIndex(row=>row.dataset.staffId===focusId):-1;const pager=document.createElement('div');pager.className='table-pager';const prev=document.createElement('button'),next=document.createElement('button'),count=document.createElement('span');prev.type=next.type='button';prev.textContent=labels.previous;next.textContent=labels.next;count.setAttribute('aria-live','polite');pager.append(prev,count,next);section.append(pager);function draw(){if(!section.getClientRects().length)return;rows.forEach(r=>r.hidden=false);const height=Math.max(...rows.map(r=>r.getBoundingClientRect().height+12),40),size=Math.max(1,Math.min(8,Math.floor((innerHeight-rows[0].getBoundingClientRect().top-100)/height))),pages=Math.ceil(rows.length/size);if(focusRow>=0){page=Math.floor(focusRow/size);focusRow=-1;}page=Math.min(page,pages-1);rows.forEach((r,i)=>r.hidden=i<page*size||i>=(page+1)*size);pager.hidden=pages<=1;prev.disabled=page===0;next.disabled=page===pages-1;count.textContent=`${page+1} / ${pages}`;}prev.onclick=()=>{page--;draw();};next.onclick=()=>{page++;draw();};window.addEventListener('resize',draw);draw();});
