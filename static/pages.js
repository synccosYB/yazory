...
if(familySupporterTable){familySupporterTable.id='family-supporters-table';familySupporterTable.classList.add('family-supporters-table');let compactedActions=false;for(const row of familySupporterTable.tBodies[0]?.rows||[]){const phoneCell=row.cells[2],digits=phoneCell?.textContent.replace(/\D/g,'');if(digits?.length===10){phoneCell.textContent=`(${digits.slice(0,3)}) ${digits.slice(3,6)}-${digits.slice(6)}`;phoneCell.dir='ltr';}const editCell=row.cells[4],editLink=editCell?.querySelector(':scope > .row-actions > a[href*="/edit"]'),nameBox=row.cells[0]?.querySelector('.supporter-name');if(editLink&&nameBox){const editLabel=editLink.textContent.trim(),supporterName=nameBox.querySelector('a')?.textContent.trim()||'';editLink.className='supporter-row-edit';editLink.textContent='✎';editLink.title=editLabel;editLink.setAttribute('aria-label',`${editLabel} — ${supporterName}`);nameBox.querySelector('a')?.after(editLink);if(!editCell.querySelector('.supporter-contact-actions')){editCell.remove();compactedActions=true;}}}if(compactedActions)familySupporterTable.tHead?.rows[0]?.lastElementChild?.remove();const heading=familySupporterTable.closest('section')?.querySelector('.supporter-summary-heading');if(heading){const search=document.createElement('input'),label=document.documentElement.lang==='yi'?'זוכן העלפער':document.documentElement.lang==='he'?'חיפוש תומכים':'Search supporters',count=heading.lastElementChild;search.type='search';search.placeholder=label;search.setAttribute('aria-label',label);search.autocomplete='off';search.dataset.tableSearch=familySupporterTable.id;count?.classList.add('family-supporter-count');const tools=document.createElement('div');tools.className='family-supporter-tools';heading.append(tools);if(count)tools.append(count);tools.append(search);}}
