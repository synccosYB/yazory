...
const communicationMetricData=document.getElementById('communication-callbacks'),communicationTable=document.getElementById('communications-supporters-table'),communicationSearch=document.querySelector('[data-table-search="communications-supporters-table"]');
const mailboxSearch=document.querySelector('[data-mailbox-search]'),mailboxList=document.querySelector('[data-mailbox-list]');if(mailboxSearch&&mailboxList){mailboxSearch.addEventListener('input',()=>{const query=mailboxSearch.value.trim().toLowerCase();for(const message of mailboxList.querySelectorAll('.mailbox-message'))message.hidden=query&&!message.textContent.toLowerCase().includes(query);});}const composeButton=document.querySelector('[data-mailbox-compose]'),composePanel=document.querySelector('[data-mailbox-compose-panel]'),composeClose=document.querySelector('[data-mailbox-compose-close]');if(composeButton&&composePanel){composeButton.addEventListener('click',()=>{composePanel.hidden=false;composePanel.querySelector('select')?.focus();});composeClose?.addEventListener('click',()=>{composePanel.hidden=true;});}
const mailboxFolders=[...document.querySelectorAll('[data-mailbox-folder]')];if(mailboxFolders.length&&mailboxList){const showFolder=folder=>{mailboxFolders.forEach(link=>link.classList.toggle('selected',link.dataset.mailboxFolder===folder));if(folder==='sent'){const tools=document.querySelector('#communication-tools'),history=document.querySelector('#communication-history');if(tools)tools.open=true;history?.scrollIntoView({behavior:'smooth',block:'start'});return;}mailboxSearch.value='';for(const message of mailboxList.querySelectorAll('.mailbox-message')){const category=message.dataset.mailboxCategory||message.id.replace('mailbox-','');message.hidden=folder!=='inbox'&&category!==folder;}mailboxList.closest('.mailbox-main')?.scrollIntoView({behavior:'smooth',block:'start'});};mailboxFolders.forEach(link=>link.addEventListener('click',event=>{event.preventDefault();showFolder(link.dataset.mailboxFolder);}));}
...
if(communicationMetricData&&communicationTable&&communicationSearch){const callbackData=JSON.parse(communicationMetricData.textContent),callbackIds=new Set(callbackData.callbacks),overdueIds=new Set(callbackData.overdue),metricLinks=[...document.querySelectorAll('[data-communication-filter]')],openPanel=section=>{const index=candidates.indexOf(section),control=document.querySelector('.page-panels')?.children[index];if(control?.tagName==='BUTTON')control.click();};metricLinks.forEach(link=>link.addEventListener('click',event=>{event.preventDefault();const filter=link.dataset.communicationFilter;for(const row of communicationTable.tBodies[0].rows){const supporterLink=row.querySelector('a[href^="/supporters/"]'),contactId=Number(supporterLink?.getAttribute('href').match(/\/supporters\/(\d+)/)?.[1]);row.dataset.metricMatch=String(filter==='all'||filter==='callbacks'&&callbackIds.has(contactId)||filter==='overdue'&&overdueIds.has(contactId));}communicationSearch.value='';communicationSearch.dispatchEvent(new Event('input'));metricLinks.forEach(item=>item.classList.toggle('selected',item===link));const section=communicationTable.closest('section');openPanel(section);section?.scrollIntoView({behavior:'smooth',block:'start'});}));document.querySelector('.communication-metrics>a:last-child')?.addEventListener('click',event=>{event.preventDefault();const section=[...document.querySelectorAll('main>section.card')].at(-1);openPanel(section);section?.scrollIntoView({behavior:'smooth',block:'start'});});}
