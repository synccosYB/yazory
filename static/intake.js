...
function review(){repeaters.forEach(r=>r.sync());const root=document.getElementById('intake-review');root.replaceChildren();const entries=[['children',form.elements.children_count.value],['rent',form.elements.rent.value],['food',form.elements.food.value],['income',[form.elements.his_income.value,form.elements.her_income.value,form.elements.other_income.value].filter(Boolean).join(' + ')],['assistance',String(repeaters[0].count())],['accounts',String(repeaters[1].count())]];entries.forEach(([key,value])=>{const line=document.createElement('div'),label=document.createElement('span'),v=document.createElement('strong');label.textContent=root.dataset[key];v.textContent=value||root.dataset.blank;line.append(label,v);root.append(line);});}
function show(i){step=i;pages.forEach((p,n)=>p.hidden=n!==i);tabs.forEach((t,n)=>{t.classList.toggle('selected',n===i);t.setAttribute('aria-current',n===i?'step':'false');});document.getElementById('intake-back').disabled=i===0;document.getElementById('intake-next').hidden=i===pages.length-1;document.getElementById('intake-progress').textContent=`${i+1} / ${pages.length}`;if(i===pages.length-1)review();}
...
if(totals){
 ...
 function updateTotals(dirty=false){
  ...
  const values={children:val('children_count')||totals.dataset.blank,costs,income,help,gap:costs===null?null:Math.max(0,costs-(income??0)-usableHelp),accounts:JSON.parse(form.elements.accounts_json.value||'[]').length};
