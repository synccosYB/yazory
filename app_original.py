...
class Family(db.Model):
    ...
    fund_name = db.Column(db.String(160), nullable=False, default='')
...
class CharityDonor(db.Model):
    ...
    # An internal label for an anonymous donor; imports never overwrite it.
    local_name = db.Column(db.String(300), nullable=False, default='')
...
<div style="margin-top:6px;color:#d4dfeb;font-size:12px;line-height:1.5">A circle of support.</div>
    ...
    def ensure_schema(run_data_migrations=False):
        ...
        schema_inspector = inspect(db.engine)
        if 'assistance_application' in schema_inspector.get_table_names():
            application_columns = {
                column['name'] for column in schema_inspector.get_columns('assistance_application')
            }
            if 'fund_name' not in application_columns:
        ...
        donor_columns = {column['name'] for column in inspect(db.engine).get_columns('charity_donor')}
